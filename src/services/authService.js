...
const DUMMY_HASH = bcrypt.hashSync(
  process.env.BCRYPT_DUMMY_SECRET || "fleeswap_dummy_secret",
  10,
);
...
const generarVerificationToken = () => {
  ...
  const verificationTokenHash = crypto
    .createHash("sha256")
    .update(verificationTokenPlano)
    .digest("hex");

  return {
    verificationTokenPlano,
    verificationTokenHash,
    verificationTokenExpiry: new Date(Date.now() + VERIFICATION_TOKEN_TTL_MS),
  };
};

const enviarEmailDeVerificacion = async ({ email, nombre, token }) => {
  const frontendURL = process.env.FRONTEND_URL || "http://localhost:5173";
  const verifyURL = `${frontendURL}/verify-email?token=${token}`;

  await enviarEmail({
    to: email,
    subject: "Fleeswap - Verifica tu email",
    html: `
      <h2>Hola ${nombre},</h2>
      <p>Gracias por registrarte en Fleeswap.</p>
      <p>Confirma tu email para terminar de activar tu cuenta.</p>
      <p><a href="${verifyURL}" style="display:inline-block;padding:12px 24px;background:#2563EB;color:#fff;text-decoration:none;border-radius:6px;">Verificar email</a></p>
      <p>Si no creaste esta cuenta, puedes ignorar este mensaje. El link expira en 24 horas.</p>
...
const register = async ({ nombre, apellido, fechaNacimiento, email, password }) => {
  ...
  await enviarEmailDeVerificacion({
    email: user.email,
    nombre: user.nombre,
    token: verificationTokenPlano,
  });
...
const refresh = async (refreshToken) => {
  ...
  // Segunda linea de defensa: verifica que el token no fue revocado en DB.
  // Cubre el caso de manipulacion directa de DB o logout previo.
...
const solicitarResetPassword = async (email) => {
  ...
  // Si no existe, no hacemos nada; respuesta identica al caso exitoso.
  if (!user) return;
...
const resendVerificationEmail = async (userId) => {
  const user = await userRepository.findByIdConVerificationToken(userId);

  if (!user) throw new AppError("Usuario no encontrado", 404);
  if (user.isVerified) throw new AppError("Tu email ya fue verificado", 400);
  ...
  await userRepository.actualizarVerificationToken(
    user._id,
    verificationTokenHash,
    verificationTokenExpiry,
  );
